...
class InternalModal extends Component {
	...
	async handleTransfer() {
		const { onSuccess, onFailure, onClose, onProcessing } = this.props
		onClose()
		const {
			// senderAccountID,
			receiver,
		...
		const data = {
			...
			transactionMessage: detail ? detail : '(empty)',
			fromAccountId: getAccountIDFromStorage(),
		}
		onProcessing()
		const res = await api.post('/transactions/transferring-interbank', data)
		if (res.error) {
			const { error } = res
			onFailure(error)
		} else {
