    ...
    <div class="border-container" style="padding:20px;">
        <div class="table-responsive">
            <table class="table table-striped border-table">
                ...
                <tbody class="tbody" v-if="listReceiveTransactionToShow[0]">
                    <tr v-for="(value,index) in listReceiveTransactionToShow" :key="index">
                        <td width="5%">{{start + index}}</td>
                        <td>
                            <span>{{value.from_fullname}}</span><br>
                            <span v-if="value.from_bank_id == 'EIGHT.Bank'">{{value.from_account_id.replace(/^(\d{2})?(\d{4})?(\d{4})?(\d{4})?/g, '$1 $2 $3 $4')}}</span>
                            <span v-else>{{value.from_account_id.replace(/^(\d{4})?(\d{4})?(\d{4})?(\d{4})?/g, '$1 $2 $3 $4')}}</span>
                        </td>
                        <td>
...
export default {
    ...
    methods:{
        async loadData() {
            setUrlDefaultWithTransactionType(this.index,this.limit,this.accountId,'receive')
